...
document.addEventListener('DOMContentLoaded', function () {
    ...
    // ── Collapse indicator sync ──────────────────────────────────────
    document.addEventListener('shown.bs.collapse', function (e) {
        var indicator = e.target.previousElementSibling.querySelector('.collapse-indicator');
    ...
    sections.forEach(function (section) {
        var tbody = document.querySelector('tbody[data-section="' + section + '"]');
        var paginationBar = document.getElementById('pagination-' + section);

        if (!tbody || !paginationBar) return;

        var rows = Array.from(tbody.querySelectorAll('tr.queue-row'));

        // Nothing to paginate if 10 or fewer rows
